...
namespace Unity.LEGO.Behaviours.Actions
{
    public class ExplodeAction : Action
    {
        ...
        private void CustomExplode()
        {
            ...
            // Send all bricks in scope flying.
                foreach (var brick in m_ScopedBricks.Take((int)(m_ScopedBricks.Count * 0.5f)))
            {
                var rigidBody = brick.gameObject.GetComponent<Rigidbody>();
                if (!rigidBody)
                {
                    ...
                    brick.transform.position += new Vector3(Random.Range(-ofset, ofset), Random.Range(-ofset, ofset), Random.Range(-ofset, ofset));
